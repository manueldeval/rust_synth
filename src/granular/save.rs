...
pub mod event;
use event::*;
use dsp::{core::{Buffer, Module, SharedBuffer, StereoGenerator}, modules::{ops::{Adder, Multiplier}, oscillators::{BaseOscillator, Samples}, wave::{SineWave, TriangleWave, Wave}}};
pub struct Granular {
...
impl Granular {
    pub fn new(recv: EventReceiver) -> Self {
        let mut wav = Samples::new("/home/deman/Téléchargements/tribal.wav");
    ...
    pub fn handle_event(&self){
        match self.event_receiver.receive() {
            Some(Event::NoteOn(_channel,midi_note, velocity)) => {
            ...
            Some(Event::NoteOff(_channel,midi_note)) => {
                let mut osc = self.main_osc.try_borrow_mut().unwrap();
                (*osc).set_frequency(440.0 * (2.0f32).powf( (midi_note as f32- 69.0)/12.0));
                (*osc).set_level(0.0);
            }
