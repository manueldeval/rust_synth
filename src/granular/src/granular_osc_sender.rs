...
impl GranularOscMessageSender {
    pub fn new(local_osc_address: String,remote_osc_address: String, gui_event_receiver: &GuiEventReceiver) -> Self {
    ...
    pub fn start(&mut self) -> Result<(), GranularOscSenderError> {
        ...
        let local_addr = SocketAddrV4::from_str(&self.local_osc_address)
            .map_err(|_| GranularOscSenderError::NotAValidAddress(self.local_osc_address.to_owned()))?;
        let remote_addr = SocketAddrV4::from_str(&self.remote_osc_address)
            .map_err(|_| GranularOscSenderError::NotAValidAddress(self.remote_osc_address.to_owned()))?;
